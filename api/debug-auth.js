...
module.exports = async function handler(req, res) {
    ...
    try {
        if (!SPREADSHEET_ID || !privateKey || !clientEmail) {
            return res.status(200).json({ error: "Missing Config", hasSpreadsheetId: !!SPREADSHEET_ID, hasPrivateKey: !!privateKey, hasClientEmail: !!clientEmail });
        }
        ...
        res.status(200).json({
            ...
            status: "Healthy",
            google: { status: "Connected", rowCount: (response.data.values || []).length },
            stripe: { status: process.env.STRIPE_SECRET_KEY ? "Configured" : "Missing" }
        });

    } catch (err) {
        res.status(500).json({ status: "Error", message: err.message, type: err.message.includes('1E08010C') ? 'Decoder Failure' : 'Other' });
    }
};
